...
| M1 規則層 | 完成 |
| M2 策略與 Bot 層 | 翻前使用預設組合表；翻後三街已接牌面分類與 equity 工程基準，顧問頻率待匯入 |
| M3 桌面應用 UI | V.1 App shell 完整（頂部列與狀態列）、面板 A／B／C／E／G 可用；D 有翻前矩陣與翻後節點表，F 未做 |
...
- **翻後規則節點（UI 規格 D.5）第一階段已完成**：翻牌／轉牌／河牌，分成
  無人下注與面對下注；列出過牌、跟注、1/3、2/3、1 倍底池、蓋牌六個動作，
  不成立的動作仍顯示並註明原因。牌面使用花面、聽花面、彩虹面、彩虹公對面、
  聽花公對面、三條面，以及可重疊的乾燥／濕潤標籤。
- 模擬使用 `equityTexture/v2-unapproved` 工程基準，會依可見牌、存活對手數、
  底池賠率與牌面質地產生分佈；顧問頻率尚未匯入，翻後規則目前唯讀。
- **策略庫（D.9）沒做**：儲存、命名、切換、匯出都還沒有，覆寫目前只活在
  當前設定裡（會隨 run 快照存下來，但不會單獨存成一份可重用的策略檔）。

**M2 待做**：正式翻後策略表（顧問內容）、7 組官方人格、多人範圍 Equity 與後續校準。

> **翻後目前是具版本的工程基準，不是顧問簽核內容。** `equityTexture/v2-unapproved`
> 對隨機合法對手範圍做固定樣本的 Monte Carlo equity，依公平勝率、底池賠率與牌面質地分桶；
> 主動動作使用 1/3、2/3、1 倍底池，乾面偏小注、濕面偏大注；
> 它解決「所有玩家一路 check 到攤牌」的執行缺口，但正式報表仍須標示未校準。
>
> **21 個 Bot 參數目前有 8 個會改變決策。** 其餘的已依核心規格 4.3 宣告，
