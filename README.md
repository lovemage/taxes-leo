...
| M1 規則層 | 完成 |
| M2 策略與 Bot 層 | 翻前使用預設組合表；翻後已接 equity heuristic 工程基準，顧問規則待匯入 |
...
- **翻後規則清單（UI 規格 D.5）沒做**，因為顧問的可編輯規則表還沒進來。
  模擬目前使用 `equityHeuristic/v1-unapproved` 工程基準，會依可見牌、存活對手數、
  底池賠率與合法行動產生下注／跟注／加注／棄牌分佈。
- **策略庫（D.9）沒做**：儲存、命名、切換、匯出都還沒有，覆寫目前只活在
  當前設定裡（會隨 run 快照存下來，但不會單獨存成一份可重用的策略檔）。

**M2 待做**：正式翻後策略表（顧問內容）、7 組官方人格、多人範圍 Equity 與後續校準。

> **翻後目前是具版本的工程基準，不是顧問簽核內容。** `equityHeuristic/v1-unapproved`
> 對隨機合法對手範圍做固定樣本的 Monte Carlo equity，依公平勝率與底池賠率分桶；
> 它解決「所有玩家一路 check 到攤牌」的執行缺口，但正式報表仍須標示未校準。
>
> **21 個 Bot 參數目前有 7 個會改變決策。** 其餘的已依核心規格 4.3 宣告，
