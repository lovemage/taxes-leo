...
- 自動補位進場的 Bot 沿用該座位既有的 persona × level 設定，Bot 組合在整個 run 中維持不變，`RunManifest` 的 Bot 快照因此仍然成立。
...
- 破產離桌與補位：dead small blind、dead button、BB 不被跳過、補位關閉時人數不足 6 即結束桌次、使用者破產即結束桌次、自動補位沿用座位人格設定（測試向量 R15–R23）。
- Forced bets：四種 ante 模式、無／單／double straddle、rake 開關、cap 與 no-flop-no-drop。
- 規則：fold、check、call、完整加注、短額 all-in、split pot、odd chip、showdown；[`德州撲克規則細則.md`](德州撲克規則細則.md) 第九章的測試向量 R1–R23 全數通過。
